...
public class StatusHandler {
	...
	public static void verifyPlayerHasStatus(ICasinoTable table, UUID playerId, PlayerStatus expectedStatus) {
		Objects.requireNonNull(table);
		Objects.requireNonNull(playerId);
		Objects.requireNonNull(expectedStatus);
		ICasinoPlayer player = table.getPlayers().stream().filter(searchedPlayer -> searchedPlayer.getId() == playerId).findFirst().orElseThrow();
